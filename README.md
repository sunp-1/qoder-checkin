...
- 2026-09-26 起，服务端要求请求携带**设备标识**才下发每日活动。v1.2.0 起本工具会自动向**本机安装的 Qoder 客户端**借用这个标识（运行它自带的一个官方组件，客户端自己每小时也在跑它；仍不写任何属于本工具的文件）。因此：
  - 在装有（或正运行）Qoder 客户端的 Windows 上开箱即用，安装位置不限（默认目录 / 注册表 / 运行中进程三条路都试）；
  - 实在找不到时可用环境变量 `QODER_UMID_EXE` 指到客户端安装目录里的对应组件，或干脆不管 —— 工具会退回普通请求并明确提示；
  - 在非 Windows 上只用 `--token` 时拿不到设备标识，若一直报 `pending`（活动未下发），请在装有客户端的 Windows 机器上运行。
